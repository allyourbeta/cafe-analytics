...
def build_items_revenue_query(start_date, end_date, item_type='all'):
    """
    Build a query for items-by-revenue endpoint.

    Returns items sorted by revenue, with optional item type filtering.
    """
    qb = QueryBuilder()
    qb.select('''
        i.item_id,
...
def build_items_profit_query(start_date, end_date, item_type='all'):
    """
    Build a query for items-by-profit endpoint.

    Returns items sorted by total profit, with optional item type filtering.
    """
    qb = QueryBuilder()
    qb.select('''
        i.item_id,
...
def build_categories_revenue_query(start_date, end_date):
    """
    Build a query for categories-by-revenue endpoint.
    ...
    qb.from_transactions_with_items()
    qb.add_date_range_filter(start_date, end_date)
    qb.group_by('i.category')
    qb.order_by('revenue DESC')

    return qb.build()


def build_categories_profit_query(start_date, end_date):
    """
    Build a query for categories-by-profit endpoint.

    Returns categories sorted by total profit.
    """
    qb = QueryBuilder()
    qb.select('''
        i.category,
...
def build_top_items_query(start_date, end_date, limit=25):
    """
    Build a query for top-items endpoint.

    Returns top items by revenue, with optional limit.
    """
    qb = QueryBuilder()
    qb.select('''
        i.item_id,
